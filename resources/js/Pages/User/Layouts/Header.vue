...
const searchProductByTitle = async () => {
    ...
    try {
        await router.post(route('products.titles'), formData);
    } catch (err) {
        console.log(err)
    }
}
...
<template>
    <nav class="bg-white border-gray-200 dark:bg-gray-900">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
            ...
            <div v-if="canLogin" class="flex items-center gap-4 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
                ...
                <!-- Search product -->
                <form @submit.prevent="searchProductByTitle">
                    <div class="relative">
                        <input type="text" class="p-1" v-model="title" />
                        <button type="submit" class="absolute right-1 top-1/2 -translate-y-1/2">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
